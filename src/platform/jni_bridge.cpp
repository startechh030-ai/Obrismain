...
Java_com_obris_ObrisActivity_nativeCreate(
    ...
    // 1. Get AAssetManager from assetManager (2nd argument)
    if (assetManager) {
    ...
    // 3. Get ANativeWindow from surface (1st argument)
    ...
    config.env = env;      // Pass valid JNIEnv*
    ...
    LOGI("Obris native engine created (%dx%d)", width, height);
...
Java_com_obris_ObrisActivity_nativeDestroy(
    ...
    LOGI("Obris native engine destroyed");
}
